...
**Pitfall when re-testing:** `dQw4w9WgXcQ` (Rick Astley — yt-dlp's own
canonical test video, requested constantly by every yt-dlp install on
earth) succeeded from this server even while the block was fully active on
every other video tested. It's likely served through a permissive/heavily
cached path and is not a representative test. Use an ordinary, non-famous
video when checking whether the block has lifted — a false "it's fixed"
reading from this one video already happened once during this deployment.
